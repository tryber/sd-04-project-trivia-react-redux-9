...
class Question extends Component {
  ...
  activeHurry(seconds) {
    const { hurry } = this.state;
    if (seconds <= 10 && !hurry) {
      return this.setState({ hurry: true });
    }
    return null;
  }
  ...
  startTimer() {
    ...
    const timerFunc = setInterval(() => {
      ...
      this.activeHurry(seconds);

      if (seconds <= 1) {
        this.setState({ disabled: true, hurry: false });
        clearInterval(timer);
      }
      if (seconds > 0) {
        this.setState((state) => ({
      ...
      <div className="question-page-container">
        ...
        <div className="question-and-answers">
          ...
          <div className="timer-and-next-button">
            <div className={hurry ? 'hurry timer' : 'timer'}>Tempo: {this.state.seconds}</div>
            <button
              type="button"
